...
void Urgc::process_on_thread()
{
	...
	while (process_thread_running) {
		...
		*/
		...
		long long derefCount = 0;
		long long guardCount = 0;
		long long deguardCount = 0;
		...
		if (size > 0) {
			...
			costMs = ms3 - ms;
			/*
			printf("处理事件[%d]结束,耗时:%lld ms %lldms. cost ref:%d,%lldus, deref:%d, %lldus guard:%d, %lldus degurad:%d, %lldus\n", 
