...
void ImageViewerState::showTooltip(const std::string& text, float x, float y) {
	tooltipText = text;
	tooltipX = x;
	tooltipY = y + 22;
	if (tooltipOverlay) {
...
void ImageViewerState::hideTooltip() {
	tooltipText.clear();
	if (tooltipOverlay) {
	...
	RINS(root.get()) {
		...
		RN(LayoutLinear) {
			...
			RN(View) {
				...
				if (o.created) {
					auto t = rttr::type::get_by_name("View");
					auto v = t.create();
...
void ImageViewerState::openFile(const std::string& path) {
	currentPath = path;
...
void ImageViewerState::onThumbnailSelect(int index) {
...
void ImageViewerState::onOpenFile() {
