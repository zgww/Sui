...
// ---------- 动作 ----------
void SpriteEditorState::onNewAnim() {
...
void SpriteEditorState::onDeleteAnim() {
...
void SpriteEditorState::onSelectAnim(int index) {
...
void SpriteEditorState::onCellClicked(int gx, int gy, bool ctrl) {
	if (!curAnim) return;
	if (ctrl) {
...
void SpriteEditorState::onOpenImage() {
...
void SpriteEditorState::onOpenSpriteFile() {
...
void SpriteEditorState::onSaveAs() {
...
void SpriteEditorState::openSpriteFile(const std::string& path) {
...
void SpriteEditorState::saveToFile(const std::string& path) {
...
// ---------- 预览 ----------
void SpriteEditorState::startPreview() {
	previewPlaying = true;
	if (previewPtr) previewPtr->play();
}

void SpriteEditorState::stopPreview() {
	previewPlaying = false;
	if (previewPtr) previewPtr->stop();
}

void SpriteEditorState::restartPreview() {
	if (previewPtr) {
		previewPtr->stop();
...
void SpriteEditorState::onGridChanged() {
	if (canvasPtr) canvasPtr->invalidDraw();
	restartPreview();
}

// ---------- 渲染 ----------
void SpriteEditorState::render() {
	auto self = Ref(this);
	auto& t = *themeIns();
...
void SpriteEditorState::styleToolBtn(Button& b) {
	b.setFontSize(13);
...
void SpriteEditorState::formField(Node& parent, const std::string& label, const std::string& key, std::function<void(const std::string&)> onChange) {
	auto self = Ref(this);
	auto& o = parent;
	RN(LayoutLinear) {
		o.direction = "row";
