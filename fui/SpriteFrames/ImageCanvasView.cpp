...
const char* ImageCanvasView::getClassName() const {
	return "ImageCanvasView";
}

void ImageCanvasView::setSrc(const std::string& path) {
	if (src == path) return;
	src = path;
...
void ImageCanvasView::zoomIn() {
	zoom *= 1.2f;
	invalidDraw();
	fireZoomChanged();
}

void ImageCanvasView::zoomOut() {
	zoom /= 1.2f;
...
void ImageCanvasView::rotate90() {
	rotation += 3.14159265358979f / 2.0f;
	invalidDraw();
	fireRotationChanged();
}

void ImageCanvasView::setZoom(float z) {
	zoom = z;
	if (zoom < 0.02f) zoom = 0.02f;
	invalidDraw();
	fireZoomChanged();
...
void ImageCanvasView::onEvent(Event* ev) {
	View::onEvent(ev);
...
void ImageCanvasView::draw_self(Canvas* canvas) {
...
void ImageCanvasView::drawCheckerboard(Canvas* canvas) {
...
bool ImageCanvasView::toImageCoord(MouseEvent* me, float* ix, float* iy) {
...
bool ImageCanvasView::getCellAt(MouseEvent* me, int* gx, int* gy) {
...
void ImageCanvasView::updateHover(MouseEvent* me) {
	int gx = 0, gy = 0;
...
void ImageCanvasView::fillRect(Canvas* canvas, float x, float y, float w, float h, int color) {
...
void ImageCanvasView::drawSpriteOverlay(Canvas* canvas, float imgW, float imgH) {
	if (!info) return;
	float cellW = info->getCellWidth((int)imgW);
	float cellH = info->getCellHeight((int)imgH);
...
void ImageCanvasView::showContextMenu(float clientX, float clientY) {
...
void ImageCanvasView::fireZoomChanged() {
	if (onZoomChanged) onZoomChanged->invoke(zoom);
}

void ImageCanvasView::fireRotationChanged() {
