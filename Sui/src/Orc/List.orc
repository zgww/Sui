...
class List {
    ...
    //交换两个指针
    extern bool swap(int fIndex, int tIndex)

    extern void resize(int size)

    extern void set(int idx, Object *obj)
    ...
    List@ span(int fromIndex, int toIndex){
...
class PointerArray{
    ...
    void insert(void *ptr, int i){
        ...
        if (i < self.length){
            memmove((void*)(self.data + i + 1), self.data+i,  _getPointerSize() * (self.length - i - 1))
        }
        self.data[i] = ptr;
        self.length = self.length + 1
    }
