...
class SglEditorSceneViewCallback extends ViewCallback {
    SglEditor@ sglEditor
    void afterDrawSelf(View *div, Canvas *canvas){
        ...
        self.sglEditor.toolMgr.draw2d(canvas)
    }
}

class SglEditor extends Listener{
    ...
    Drag@ drag = new Drag()
    SglEditorSceneViewCallback@ viewCb = new SglEditorSceneViewCallback()
                    ...
                    mkSglSceneView(o, 0).{
                        ...
                        if o.matl != self.matl {
                            ...
                            new PerspectiveCamera().{
                                tmpCam = o
                                o.name = str("测试gizmo相机")
                                o.updateProjectionMat()
                                o.updateViewMat()

                                o.position.set(-100, -0, 0)
                                scene.appendChild(o)
                            }

                            // new Mesh(). {
                            //     GeometryLine@ geom = new GeometryLine()
                            //     geom.buildCameraHelper(tmpCam)
                            //     o.geometry = geom
                            ...
                            mkBillboard(scene, 0).{
                                o.name =  str("billboad")
                                GeometryRect@ geom = new GeometryRect()
                                geom.width = 320
                                geom.height = 320
                                // geom.depth = 400
                                geom.build()
                                ...
                                o.material = new Material()
                                o.material.load("../asset/billboard.matl.json")
                                Tex2d@ tex = mkTex2dByPathCstr("../asset/gizmo/camera.png")
                                o.material.setUniformTex2d("tex", tex)

                                o.geometry = geom

                                o.position.set(100, 100, 0)
                            }
                            if 0 {
                            ...
                            o.drag.onDrag = ^void(Drag *d){
                                ...
                                if d.mouseDownButton == 1 {
                                    ...
                                    self.sceneView.camera.position.add3fLocal(d.deltaPos.x, d.deltaPos.y, 0)
                                }
    ...
    void showOutline(){
        ...
        self.sceneView.drawCtx.{
            ...
            self.outlineFx.drawMask(o, self.heightmap)
    ...
    void testShowMatl(){
        ...
        matl.load("../asset/box.matl.json")
        new MaterialInsp().{
            o.showWindow(matl)
        }
    }
    void showWindow(){
        Window@ win = new Window() //先创建窗口，初始化opengl环境
        if 1 {
            self.testShowMatl()
            return
        }
