...
from app.point_archive.catalog import as_catalog
...
def run():
    with tempfile.TemporaryDirectory(prefix="fms-capacity-") as tmp:
        ...
        document = as_catalog(document)
        directory = next(d["id"] for d in document["directories"] if d.get("legacy_id") == "EQ-00001" and d["role"] == "equipment")
        ...
        def request(i):
            ...
            if i % 2:
                params["directory_id"] = directory
            started = time.perf_counter()
            result = client.get("/api/point-archive/catalog/points", params=params)
        ...
        payload = store.catalog_points(page_size=50, name="油温", bound=False, bound_ids=set())
        components = {
            "store_global_filter_median_ms": timed(lambda: store.catalog_points(page_size=50, name="油温", bound=False, bound_ids=set())),
            "store_equipment_filter_median_ms": timed(lambda: store.catalog_points(page_size=50, name="油温", bound=False, bound_ids=set(), directory_id=directory)),
